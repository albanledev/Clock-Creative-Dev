...
import { Clock } from "../canvas/shapes/Clock";
...
export default class Scenario extends Scene {
  constructor(id) {
    super(id);
    this.clock = new Clock(this.width / 2, this.height / 2);
    this.drawGradation();
    this.arcs = [];
    this.nArcs = 10;
      ...
      this.arcs.forEach((arc) => {
        if (arc) {
          if (this.params["is-update"]) {
            if (this.globalContext && this.globalContext.time) {
              arc.update(
                this.globalContext.time.delta / 1000,
                this.params.speed
              );
            } else {
              console.error("globalContext or globalContext.time is undefined");
            }
          }
